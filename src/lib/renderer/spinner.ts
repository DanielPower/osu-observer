...
export class Spinner extends Container {
	...
	constructor({
		...
		if (overallDifficulty <= 5) {
			requiredRPM =
				COMPLETE_RPM_MIN + (COMPLETE_RPM_MID - COMPLETE_RPM_MIN) * (overallDifficulty / 5);
		} else {
			requiredRPM =
				COMPLETE_RPM_MID + (COMPLETE_RPM_MAX - COMPLETE_RPM_MID) * ((overallDifficulty - 5) / 5);
		}
	...
	update(time: number, rotation: number) {
		...
		if (completionProgress > 0) {
			...
			this.progressCircle.stroke({ width: 6, color: progressColor, alpha: 0.9 });
		}
	}
}
